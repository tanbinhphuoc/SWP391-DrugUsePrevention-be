        ...
        public async Task<(CourseRegistrationDto, string)> RegisterCourseAsync(RegisterCourseDto dto, int userId, string clientIp, HttpContext context)
        {
            ...
            if (existing != null)
            {
                ...
                if (string.IsNullOrWhiteSpace(existing.PaymentStatus))
                {
                    ...
                    existing.Status = course.Price == 0 ? "FREE" : "PENDING_PAYMENT";
                    existing.Amount = course.Price;
                    existing.PaymentStatus = course.Price == 0 ? "SUCCESS" : "PENDING";
            ...
            var registration = new CourseRegistration
            {
                ...
                Status = course.Price == 0 ? "FREE" : "PENDING_PAYMENT",
                Amount = course.Price,
                PaymentStatus = course.Price == 0 ? "SUCCESS" : "PENDING"
            };
